...
def check(colors, response):
    try:
        ...
        verUrl = urllib.urlopen('http://www.alexlaird.net/projects/ontheside/programs/files/fsupdates/fsver.ldf')
...
def getUpdate(fastSearchDir, colors, preUpdateMessage, postUpdateMessage):
    try:
        ...
        sys.stdout.flush()
        updateUrl = urllib.urlopen('http://www.alexlaird.net/projects/ontheside/programs/files/fsupdates/updates' + str(PYTHON_VERSION) + '.ldf')
        filesToGet = []
        filesToDelete = []
        filesToMove = []
        foldersToGet = []
        ...
        for newFile in filesToGet:
            serverFile = urllib.urlopen('http://www.alexlaird.net/projects/ontheside/programs/files/fsupdates/' + newFile[0]).read()
